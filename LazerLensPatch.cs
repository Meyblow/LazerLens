...
namespace LazerLens
{
    ...
    {
        public static event Action<ScoreInfo, bool>? OnScoreImported; // score, passed
        ...
        public static void Install(IOsuCcPluginHost host)
        {
            ...
            // 5. Hook ResultsScreen.LoadComplete (Catch computed PP & server stats)
            var resultsType = typeof(ResultsScreen);
            var loadComplete = resultsType.GetMethod("LoadComplete", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            if (loadComplete != null)
                host.AddPatch(loadComplete, typeof(LazerLensPatch), nameof(ResultsLoadCompletePostfix), MethodType.Postfix);

            DebugLog($"LazerLensPatch installed: import={importMethod != null}, conclude={concludeMethod != null}, restart={restartMethod != null}, exit={performExitMethod != null}, results={loadComplete != null}");
        }
        ...
        private static void ImportScorePostfix(Player __instance, Score score)
        {
            try
            {
                ...
                OnScoreImported?.Invoke(score.ScoreInfo, true);
            }
            catch (Exception ex)
            {
        ...
        private static void ConcludeFailedScorePostfix(Player __instance, Score score)
        {
            try
            {
                DebugLog($"ConcludeFailedScorePostfix: instance={__instance?.GetType().Name}, score={score != null}");
                if (__instance == null || score?.ScoreInfo == null) return;
                ...
                OnScoreImported?.Invoke(score.ScoreInfo, false);
            }
            catch (Exception ex)
            {
        ...
        private static void OnRestartPrefix(Player __instance)
        {
            ...
            if (__instance != null)
                recordUnpassedScore(__instance);
        }
        ...
        private static void recordUnpassedScore(Player player)
        {
            try
            {
                ...
                OnScoreImported?.Invoke(scoreInfo, false);
            }
            catch (Exception ex)
            {
        ...
        private static void ResultsLoadCompletePostfix(ResultsScreen __instance)
        {
            try
            {
                ...
                if (finalScore != null)
                {
                    ...
                    OnScoreImported?.Invoke(finalScore, true);
                }
